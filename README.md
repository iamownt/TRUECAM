...
Run the automl training (taking NSCLC as an example)
```angular2html
cd truecam
python automl_eat.py \
    --model_name titan \
    --subtyping_task NSCLC \
    --sample_fraction 0.4 \
    --num_folds 20 \
    --num_trials 1

cd titan_code
python generate_slide_embedding.py \
    --tile_dir ~/TCGA-OT/Patch512/TITAN/h5_files \
    --output_dir ~/TCGA-OT/Patch512/TITAN/h5_files_slide_entropy \
    --mask_pkl_path ~/TCGA-OT/models/ambpkl/titan_NSCLC_ambiguity_dict_lightgbm_0.4_balanced.pkl \
    --subtyping_task NSCLC \
    --dataset_name NSCLC \
    --proxy_train_fraction 0.4 \
    --keep_ratios 0.4 0.6 0.8 \
    --num_folds 20 \
    --ambiguity_strategy entropy
```
For NSCLC, the EAT outputs are expected here:
    ~/TCGA-OT/Patch512/TITAN/h5_files_slide_entropy/NSCLC/prop0.4_split{0..19}_{0.4/0.6/0.8}/NSCLC_titan_slide_embedding.pkl

```aiignore
Final Full run:
python TITAN/notebooks/nsclc_titan_linear_probe_conformal.py \
    --official-features TCGA_TITAN_features.pkl \
    --output-dir TITAN/notebooks
```
