...
def not_contain(k):
    if 'prob' not in k and 'label' not in k and 'logit' not in k and 'uncertainty' not in k and "patient" not in k:
        return True
    else:
        return False
...
def train(dataloader, fold, args):
    ...
    # when we use linear kernel, prov-gigapath-sngp degenerate to the prob-gigapath

    if args.gaussian_process: # we force sn with gp
        backup_weight, backup_bias = copy.deepcopy(model.classifier.weight), copy.deepcopy(model.classifier.bias)
        replace_layer_with_gaussian(container=model, signature="classifier", **GP_KWARGS_PROV_GIGAPATH)
        # model.classifier._gp_output_layer = convert_to_sn_my(model.classifier._gp_output_layer, ["Linear", "Conv2D"], args.spec_norm_bound)
        model.classifier._gp_output_layer.weight, model.classifier._gp_output_bias = backup_weight, backup_bias
    ...
    if args.ood_dataset_name is not None:
    ...
    for i in range(args.epochs):
        ...
        if args.gaussian_process:
            ...
            kwargs = {}
        train_records = train_one_epoch(train_loader, model, fp16_scaler, optimizer, loss_fn, i, args, kwargs)

        if val_loader is not None:
            val_records = evaluate(val_loader, model, fp16_scaler, loss_fn, i, args)

            # update the writer for train and val
            log_dict = {'train_' + k: v for k, v in train_records.items() if not_contain(k)}
            log_dict.update({'val_' + k: v for k, v in val_records.items() if not_contain(k)})
            log_writer(log_dict, i, args.report_to, writer)
            # update the monitor scores
            scores = val_records['micro_auroc']
    ...
    log_dict = {'test_' + k: v for k, v in test_records.items() if not_contain(k)}
...
def train_one_epoch(train_loader, model, fp16_scaler, optimizer, loss_fn, epoch, args, kwargs):
    model.train()
    # set the start time
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16 if args.fp16 else torch.float32):
            ...
            # get the logits
            logits = model(images, img_coords, **kwargs)
            # get the loss
...
def evaluate(loader, model, fp16_scaler, loss_fn, epoch, args):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            # load the batch and transform this batch
            images, img_coords, label, slide_id = batch['imgs'], batch['coords'], batch['labels'], batch["slide_id"]
            images = images.to(args.device, non_blocking=True)
            img_coords = img_coords.to(args.device, non_blocking=True)
            label = label.to(args.device, non_blocking=True).long()
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None, dtype=torch.float16):
                # get the logits
                output = model(images, img_coords, **eval_kwargs)
            ...
            if task_setting == 'multi_label':
                ...
                records["logit"][batch_idx] = logits.cpu().numpy()
                ...
                records['patient'][batch_idx] = slide_id[0]
                if args.gaussian_process:
                    records['uncertainty'][batch_idx] = uncertainty
    ...
    if task_setting == 'multi_label':
        info = 'Epoch: {}, Loss: {:.4f}, Micro AUROC: {:.4f}, Micro AUROC: {:.4f}, Micro AUPRC: {:.4f}, Micro AUPRC: {:.4f}'.format(epoch, records['loss'], records['micro_auroc'], records['micro_auroc'], records['micro_auprc'], records['micro_auprc'])
    else:
        info = 'Epoch: {}, Loss: {:.4f}, AUROC: {:.4f}, ACC: {:.4f}, BACC: {:.4f}'.format(epoch, records['loss'], records['micro_auroc'], records['acc'], records['bacc'])
