...
def remove_spectral_norm(module, name='weight'):
    # Check if the module has a parametric weight
    if not hasattr(module.parametrizations, name):
        raise ValueError(f"No parametrized '{name}' found in module")
    param_list = module.parametrizations[name]
    spectral_norm_param = next(
        (p for p in param_list if isinstance(p, _SpectralNorm)), None
    )
    if spectral_norm_param is None:
        raise ValueError("No spectral normalization found in parameterization")
    effective_weight = getattr(module, name)
    parametrize.remove_parametrizations(module, name, leave_parametrized=True)
    setattr(module, name, torch.nn.Parameter(effective_weight))
    return module

if __name__ == "__main__":
    # there are two different results of spectral norm, one is to use the nn.utils.spectral_norm, the other is to use the spectral_norm
    # note that in the first implementation, we just add a hook, but in the second implementation, we actually change the weight of the module
    base_linear = torch.nn.Linear(20, 40)
    snm = spectral_norm(base_linear, spec_norm_bound=0.7)
    # Remove spectral normalization, absorbing it into the weight
    remove_spectral_norm(snm)
    print("smm", torch.linalg.norm(snm.weight, ord=2)) # largest singular value
    # print(base_linear.weight)
    # print("snm.weight.", snm.weight)
    # print("smm", torch.linalg.norm(snm.weight, orf=2)) # largest singular value
    # print("smm", torch.linalg.norm(snm.weight,)) # Frobenius norm


    # a = torch.nn.Conv2d(1, 3, 2)
    # print(a.weight)
    # snm = spectral_norm(a, spec_norm_bound=0.2)
    # print(snm.weight.shape)
    # print("snm.weight.", snm.weight)
    # print("smm", torch.linalg.norm(snm.weight.flatten(start_dim=0), dim=0))